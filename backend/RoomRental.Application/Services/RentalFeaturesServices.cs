...
public class RentalRequestService : IRentalRequestService
{
    ...
    public RentalRequestService(ApplicationDbContext db) => _db = db;
    ...
    public async Task<YeuCauThueDto> TaoAsync(int nguoiThueId, TaoYeuCauThueDto dto)
    {
        ...
        if (post.Status != RoomRental.Domain.Enums.PostStatus.Approved) throw new Exception("Bài đăng chưa được duyệt");
...
{
    private readonly ApplicationDbContext _db;
    public RentalContractService(ApplicationDbContext db) => _db = db;
...
{
    private readonly ApplicationDbContext _db;
    public RoomReviewService(ApplicationDbContext db) => _db = db;
