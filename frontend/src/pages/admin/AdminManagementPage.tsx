...
import { FiActivity, FiArrowLeft, FiCheck, FiCheckCircle, FiChevronRight, FiEdit2, FiFileText, FiGrid, FiHome, FiLogOut, FiMenu, FiPlus, FiSearch, FiSettings, FiTag, FiTrash2, FiUnlock, FiUser, FiUsers, FiX } from 'react-icons/fi';
...
const menu = [
  ['Tổng quan', FiGrid, ROUTES.ADMIN_DASHBOARD], ['Người dùng', FiUsers, ROUTES.ADMIN_USERS], ['Phòng trọ', FiHome, '/admin/rooms'],
  ['Tin đăng', FiFileText, ROUTES.ADMIN_POSTS], ['Duyệt tin', FiCheckCircle, ROUTES.ADMIN_POST_APPROVAL], ['Danh mục', FiTag, '/admin/categories'],
  ['Tiện ích', FiSettings, ROUTES.ADMIN_AMENITIES], ['Báo cáo vi phạm', FiActivity, '/admin/reports'],
] as const;
...
const AdminManagementPage: React.FC<{ module: AdminModule }> = ({ module }) => {
  ...
  const logoutAdmin = () => { logout(); navigate(ROUTES.LOGIN); };
  ...
  const activePath = module === 'approval' ? ROUTES.ADMIN_POST_APPROVAL : `/admin/${module}`;
  const canCreate = module === 'categories' || module === 'amenities';
  const filteredRows = rows;

  return <div className="admin-shell">
    <aside className={`admin-sidebar ${menuOpen ? 'is-open' : ''}`}>
      <div className="admin-brand"><span>T</span><strong>Timnhatro<i>.vn</i></strong><button onClick={() => setMenuOpen(false)}><FiX /></button></div>
      <div className="admin-role"><b /> Khu vực quản trị viên</div>
      <nav><small>QUẢN TRỊ HỆ THỐNG</small>{menu.map(([label, Icon, href]) => <Link key={href} to={href} className={href === activePath ? 'active' : ''} onClick={() => setMenuOpen(false)}><Icon /><span>{label}</span>{href === activePath && <FiChevronRight />}</Link>)}</nav>
      <div className="admin-sidebar-footer"><Link to="/"><FiArrowLeft /> Xem trang chính</Link><button onClick={logoutAdmin}><FiLogOut /> Đăng xuất</button></div>
    </aside>
    <main className="admin-main">
      <header className="admin-topbar"><button className="admin-menu" onClick={() => setMenuOpen(true)}><FiMenu /></button><div className="admin-crumb"><span>Quản trị hệ thống</span><FiChevronRight /><b>{current.title}</b></div><div className="admin-user"><span>{user?.fullName?.charAt(0) || 'A'}</span><div><b>{user?.fullName || 'Quản trị viên'}</b><small>Quản trị viên</small></div></div></header>
      <div className="admin-content"><div className="admin-heading"><div><p>{current.eyebrow}</p><h1>{current.title}</h1><span>{current.description}</span></div>{canCreate && <button className="admin-primary" onClick={() => { setForm({ name: '', description: '', icon: '' }); setModal({ type: module === 'amenities' ? 'amenity' : 'category' }); }}><FiPlus /> Thêm mới</button>}</div>
        <div className="admin-toolbar"><label><FiSearch /><input value={query} onChange={e => setQuery(e.target.value)} placeholder={module === 'users' ? 'Tìm theo tên hoặc email...' : 'Tìm kiếm dữ liệu...'} /></label>{(module === 'posts' || module === 'rooms' || module === 'reports') && <select value={status} onChange={e => setStatus(e.target.value)}><option value="">Tất cả trạng thái</option>{module === 'posts' && <><option value="0">Chờ duyệt</option><option value="1">Đã duyệt</option><option value="2">Từ chối</option><option value="3">Đã ẩn</option></>}{module === 'rooms' && <><option value="0">Còn trống</option><option value="1">Đã thuê</option><option value="3">Tạm ngưng</option></>}{module === 'reports' && <><option value="0">Mới tiếp nhận</option><option value="1">Đang xử lý</option><option value="2">Đã xử lý</option></>}</select>}<button className="admin-refresh" onClick={load}>Làm mới</button></div>
        <section className="admin-table-card">{error && <div className="admin-alert">{error}</div>}{loading ? <div className="admin-empty">Đang tải dữ liệu...</div> : !filteredRows.length ? <div className="admin-empty"><FiFileText /><b>Chưa có dữ liệu</b><span>Thử thay đổi bộ lọc hoặc quay lại sau.</span></div> : <div className="admin-table-wrap"><table><thead><tr>{module === 'users' && <><th>Người dùng</th><th>Vai trò</th><th>Tin đăng</th><th>Trạng thái</th><th /></>}{(module === 'posts' || module === 'approval') && <><th>Tin đăng</th><th>Chủ trọ</th><th>Giá thuê</th><th>Trạng thái</th><th /></>}{module === 'rooms' && <><th>Phòng</th><th>Chủ trọ</th><th>Giá thuê</th><th>Trạng thái</th><th /></>}{(module === 'categories' || module === 'amenities') && <><th>Tên</th><th>Mô tả</th><th>Trạng thái</th><th /></>}{module === 'reports' && <><th>Nội dung báo cáo</th><th>Người gửi</th><th>Ngày gửi</th><th>Trạng thái</th><th /></>}</tr></thead><tbody>{filteredRows.map(item => <tr key={item.id}>
    ...
    {modal && <div className="admin-modal-backdrop" onClick={() => setModal(null)}><form className="admin-modal" onSubmit={handleCatalog} onClick={e => e.stopPropagation()}><div><h2>{modal.id ? 'Chỉnh sửa' : 'Thêm'} {modal.type === 'amenity' ? 'tiện ích' : 'danh mục'}</h2><button type="button" onClick={() => setModal(null)}><FiX /></button></div><label>Tên<input required value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></label>{modal.type === 'amenity' && <label>Biểu tượng<input value={form.icon} onChange={e => setForm({ ...form, icon: e.target.value })} placeholder="Ví dụ: wifi" /></label>}<label>Mô tả<textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} /></label><button className="admin-primary" type="submit"><FiCheck /> Lưu thay đổi</button></form></div>}
  </div>;
};
