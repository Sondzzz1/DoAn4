...
const DashboardContent: React.FC = () => {
  ...
  const adminRoomStats = [
    { label: 'Còn trống', value: admin?.totalRooms || 0, tone: 'green' },
    { label: 'Đã thuê', value: 0, tone: 'blue' },
    { label: 'Đang bảo trì', value: 0, tone: 'amber' },
    { label: 'Đã ẩn', value: admin?.hiddenPosts || 0, tone: 'slate' },
  ];

  const postStats = isAdmin
    ? [
        { label: 'Đã duyệt', value: admin?.approvedPosts || 0, tone: 'green' },
        { label: 'Chờ duyệt', value: admin?.pendingPosts || 0, tone: 'amber' },
        { label: 'Đã ẩn', value: admin?.hiddenPosts || 0, tone: 'slate' },
      ]
    : [
        { label: 'Đã duyệt', value: landlord?.approvedPosts || 0, tone: 'green' },
        { label: 'Chờ duyệt', value: landlord?.pendingPosts || 0, tone: 'amber' },
        { label: 'Bị từ chối', value: landlord?.rejectedPosts || 0, tone: 'rose' },
      ];
    ...
    <div className="dashboard-content-wrapper">
      ...
      <section className="dashboard-welcome">
        <div>
          <p className="dashboard-eyebrow">
            {isAdmin ? 'BẢNG ĐIỀU KHIỂN HỆ THỐNG' : 'BẢNG ĐIỀU KHIỂN CHỦ TRỌ'}
          </p>
        ...
        {isAdmin ? (
          <Link to={ROUTES.ADMIN_POST_APPROVAL} className="dashboard-primary-button">
            <FiCheckCircle />
            Duyệt tin mới
          </Link>
        ) : (
      ...
      <section className="dashboard-metrics">
        {metrics.map((metric) => (
          <article className="dashboard-metric" key={metric.label}>
            <div className={`dashboard-metric-icon ${metric.tone}`}>{metric.icon}</div>

            <div>
              <span>{metric.label}</span>
              <strong>{loading ? '—' : formatNumber(metric.value)}</strong>
      ...
      <section className="dashboard-grid">
        ...
        <article className="dashboard-panel dashboard-overview-panel">
          <div className="dashboard-panel-heading">
            <div>
              <h2>{isAdmin ? 'Tổng quan hệ thống' : 'Tình hình vận hành'}</h2>
              <p>
                {isAdmin ? 'Các chỉ số chính hiện tại' : 'Tổng hợp phòng trọ và tin đăng'}
              </p>
            </div>
          ...
          {isAdmin ? (
            <div className="dashboard-admin-summary">
              <div className="dashboard-summary-item">
                ...
                <strong>{formatNumber(admin?.totalTenants || 0)}</strong>
              </div>
              <div className="dashboard-summary-item">
                <span>Chủ trọ</span>
                <strong>{formatNumber(admin?.totalLandlords || 0)}</strong>
              </div>
              <div className="dashboard-summary-item">
                <span>Phòng trọ</span>
                <strong>{formatNumber(admin?.totalRooms || 0)}</strong>
              </div>
              <div className="dashboard-summary-item">
                <span>Tin đăng</span>
                <strong>{formatNumber(admin?.totalPosts || 0)}</strong>
              </div>
            </div>
          ) : (
            <div className="dashboard-landlord-summary">
              <div className="dashboard-summary-big">
                ...
                <strong>{formatNumber(landlord?.availableRooms || 0)}</strong>
                <small>trên tổng số {formatNumber(landlord?.totalRooms || 0)} phòng</small>
              </div>
              <div className="dashboard-summary-big">
                <span>Tin đang hiển thị</span>
                <strong>{formatNumber(landlord?.approvedPosts || 0)}</strong>
                <small>{formatNumber(landlord?.pendingPosts || 0)} tin đang chờ duyệt</small>
              </div>
            </div>
          )}
        </article>
            ...
            <article className="dashboard-panel dashboard-status-panel">
              <div className="dashboard-panel-heading">
                <div>
                  ...
                  <p>Phân bố phòng theo trạng thái</p>
                </div>
              ...
              <div className="dashboard-status-list">
                {adminRoomStats.map((item) => (
                  <div className="dashboard-status-row" key={item.label}>
                    <span>
                      <i className={`status-dot ${item.tone}`} />
                      {item.label}
                    </span>
            ...
            <article className="dashboard-panel dashboard-status-panel">
              <div className="dashboard-panel-heading">
                <div>
                  ...
                  <p>Theo dõi tình trạng kiểm duyệt</p>
                </div>
          ...
          <>
            <article className="dashboard-panel dashboard-status-panel">
              <div className="dashboard-panel-heading">
                <div>
                  <h2>Trạng thái tin đăng</h2>
                  <p>Cập nhật theo dữ liệu hiện tại</p>
                </div>
                <FiPieChart className="dashboard-panel-symbol" />
              </div>
              <div className="dashboard-status-list">
            ...
            <article className="dashboard-panel dashboard-note-panel">
              ...
              <div>
                <p className="dashboard-eyebrow">GỢI Ý</p>
                <h2>Tin đăng rõ ràng, phòng nhanh đầy</h2>
                <p>
      ...
      {isAdmin && (
        <section className="dashboard-admin-note">
          ...
          <div>
            <p className="dashboard-eyebrow">KIỂM SOÁT HỆ THỐNG</p>
            <h2>Duy trì nội dung phòng trọ minh bạch</h2>
            <p>
              Kiểm tra các tin đăng đang chờ duyệt, xử lý báo cáo vi phạm và đảm bảo thông
              tin phòng trọ trên hệ thống chính xác.
            </p>
          </div>
          <Link to={ROUTES.ADMIN_POST_APPROVAL} className="dashboard-secondary-button">
            Mở hàng đợi duyệt
            <FiArrowUpRight />
          </Link>
        </section>
